...
export const RED_ITEMS = ['연차', '촉진연차', '대체휴가', '만근휴가', '개인학습', '지정휴무'];
...
// 2026년 공휴일 마스터 데이터
export const getHolidayData = (date: Date): HolidayInfo => {
  ...
  if (date.getFullYear() === 2026) {
    if (['2-16', '2-17', '2-18'].includes(mmdd)) return { isH: true, isSat: false, name: '설날' };
    if (mmdd === '2-19') return { isH: true, isSat: false, name: '대체' };
    if (mmdd === '5-24') return { isH: true, isSat: false, name: '석탄' };
    if (mmdd === '5-25') return { isH: true, isSat: false, name: '대체' };
    if (['9-24', '9-25', '9-26'].includes(mmdd)) return { isH: true, isSat: false, name: '추석' };
    if (mmdd === '9-28') return { isH: true, isSat: false, name: '대체' };
  }

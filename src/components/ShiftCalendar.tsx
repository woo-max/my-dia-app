...
const getDutyColor = (duty: string, defaultColor: string) => {
  ...
  if (duty.includes('지정근무') || duty.includes('대기충당') || duty.includes('교번교체')) return '#3B82F6';
  if (duty.includes('휴무충당')) return '#D97706'; // 가독성 좋은 진한 주황/노랑
  return defaultColor;
};
...
export const ShiftCalendar = () => {
  ...
  // 1. 테마 색상 정의 (라이트 모드 밝기 다운 & 팝업 가시성 확보)
  const theme = {
    bg: isDarkMode ? '#000000' : '#E5E7EB', // 라이트 모드 배경: 부드러운 그레이(Gray-200)로 낮춤
    card: isDarkMode ? '#1A1A1A' : '#F9FAFB', // 라이트 모드 카드: 살짝 밝은 그레이
    text: isDarkMode ? '#FAFAFA' : '#18181B', // 글자색 대비 확실하게
    subText: isDarkMode ? '#A1A1AA' : '#52525B',
    border: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)',
    todayHeader: isDarkMode ? 'rgba(16, 185, 129, 0.4)' : 'rgba(16, 185, 129, 0.2)',
  };
  ...
  return (
    <div style={{ backgroundColor: theme.bg, color: theme.text }} className="flex flex-col h-screen select-none font-sans overflow-hidden relative">
      ...
      <header className="flex items-center justify-between px-6 py-6 shrink-0 z-10">
        <div>
          <span style={{ color: theme.subText }} className="text-[11px] font-black mb-1 block tracking-wider uppercase">{format(currentMonth, 'yyyy년')}</span>
        ...
        {isLoading ? ( <div className="flex-1 p-4 grid grid-cols-7 gap-1.5 animate-pulse"> {Array.from({ length: 35 }).map((_, i) => ( <div key={i} className="aspect-[3/4] bg-slate-300 dark:bg-neutral-800 rounded-xl" /> ))} </div>
                ...
                {eachDayOfInterval({ start: startOfWeek(startOfMonth(currentMonth)), end: endOfWeek(endOfMonth(currentMonth)) }).map(d => {
                    ...
                    return (
                        <div key={d.toString()} onClick={() => { setSelectedDate(d); setDetailData(s); openModal(setShowDetail); }}
                             style={{ backgroundColor: theme.card, borderColor: theme.border, borderWidth: '0.5px', opacity: isCurr ? 1 : 0.2 }}
                             className="relative flex flex-col transition-all overflow-hidden active:bg-slate-200 dark:active:bg-neutral-800">
                            <div style={{ backgroundColor: isT ? theme.todayHeader : 'transparent' }} className="py-1 px-1.5 flex items-center justify-between border-b border-black/5">
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center gap-0.5 overflow-hidden">
                                <span style={{ color: s.isRed ? '#EF4444' : getDutyColor(s.label, theme.text) }} className="text-[16px] font-black tracking-tighter leading-none">{s.diaNum}</span>
                                <div className="flex flex-col gap-0.5 overflow-hidden w-full items-center">
                                  {(memos[dk] || []).map((m, idx) => (<div key={idx} style={{ color: theme.subText }} className="text-[9px] font-bold truncate max-w-full px-0.5 leading-tight">{m}</div>))}
                                </div>
                            </div>
                        </div>
                    );
                ...
                <table style={{ borderColor: theme.border }} className="border-collapse w-max min-w-full text-center">
                    <thead className="sticky top-0 z-20">
                        <tr>
                            ...
                            {eachDayOfInterval({ start: isSameMonth(currentMonth, new Date()) ? new Date() : startOfMonth(currentMonth), end: endOfMonth(currentMonth) }).map(d => {
                                const isT = isSameDay(d, new Date()); const h = getHolidayInfo(d);
                                return ( <th key={d.toString()} style={{ color: h.isH ? '#EF4444' : h.isSat ? SAT_BLUE : theme.text, borderColor: theme.border, backgroundColor: isT ? theme.todayHeader : theme.card }} className="p-2 border text-[11px] min-w-[60px] relative font-sans text-center"> {isT && <span className="absolute -top-1 left-1/2 -translate-x-1/2 text-[7px] font-black bg-emerald-500 text-white px-1 rounded-sm">TODAY</span>} {format(d, 'd')}<br/>{['일','월','화','수','목','금','토'][d.getDay()]} </th> );
                            })}
                        </tr>
                    </thead>
                    <tbody>
                    {[{ id: 0, name: '나', anchorD: userAnchor.date, anchorI: userAnchor.index, isUser: true }, ...peers].map(p => (
                        ...
                        {eachDayOfInterval({ start: isSameMonth(currentMonth, new Date()) ? new Date() : startOfMonth(currentMonth), end: endOfMonth(currentMonth) }).map(d => {
                            ...
                            <td key={d.toString()} onClick={() => { setSelectedDate(d); setDetailData(s); openModal(setShowDetail); }}
                                style={{ backgroundColor: isT ? theme.todayHeader : (isE ? '#D1D5DB' : (isOff ? (isDarkMode ? '#411C1C' : '#FEE2E2') : theme.card)), borderColor: theme.border }} 
                                className="p-2 border text-center relative h-16">
                                <div style={{ color: isOff ? '#EF4444' : getDutyColor(s.label, theme.text) }} className="text-[15px] font-black">{s.diaNum}</div>
                                {closingTime && (<div style={{ color: theme.subText }} className="absolute bottom-1 right-0.5 font-black text-[12px] italic leading-none">{closingTime}</div>)}
                            </td>
                            );
                        })}
                        </tr>
            ...
            <div style={{ backgroundColor: theme.bg }} className="flex-1 flex flex-col h-full overflow-hidden">
                ...
                {['평','휴','평평','평휴','휴휴','휴평','교번'].map(t => (
                    <button key={t} onClick={() => setAllSubTab(t as any)} className={cn("px-4 py-2 rounded-xl text-xs font-black transition-all whitespace-nowrap", allSubTab === t ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-400")}>{t}</button>
                ))}
                </div>
                <div className="flex-1 overflow-auto">
                    ...
                    <tbody style={{ color: theme.text }}>
                        ...
                        <tr key={i} style={{ backgroundColor: i % 2 === 0 ? theme.card : theme.bg }} className="text-center text-[11px] font-medium border-b border-black/5">
                            ...
                            <td className="p-3">{row.content1}</td> <td className="p-3">{row.content2}</td>
                        </tr>
                        ))}
                    </tbody>
                    </table>
      ...
      {/* --- 하단 탭 바 --- */}
      <nav style={{ backgroundColor: theme.card, borderColor: theme.border }} className="mx-6 mb-6 px-2 py-2 rounded-2xl flex items-center border shadow-lg relative z-10 shrink-0">
      ...
      {showDetail && detailData && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => window.history.back()} />
            <div style={{ backgroundColor: theme.bg, color: theme.text }} className="relative z-10 w-full max-w-sm rounded-[40px] p-8 shadow-2xl flex flex-col h-[85vh] overflow-y-auto">
                ...
                <div className="flex justify-between items-end mb-8">
                  ...
                  {detailData.isUser && ( <button onClick={() => { window.history.pushState({ modal: true }, ""); setShowShiftMenu(true); }} style={{ backgroundColor: theme.card }} className="px-5 py-3 rounded-2xl font-black text-[13px] border shadow-sm text-blue-500">근무 변경</button> )}
                </div>
                <div style={{ backgroundColor: theme.card, borderColor: theme.border }} className="border rounded-[32px] p-8 flex justify-between items-center shadow-sm mb-6">
                    <div className="flex flex-col items-center gap-2"> 
                      <div style={{ backgroundColor: theme.bg }} className="w-20 h-20 rounded-[24px] flex items-center justify-center border"> 
                        <span style={{ color: detailData.isRed ? '#EF4444' : getDutyColor(detailData.label, theme.text), fontSize: detailData.diaNum?.length > 3 ? '20px' : '36px' }} className="font-black">{detailData.diaNum}</span> 
                      </div> 
                      <span style={{ color: theme.subText }} className="text-[12px] font-black uppercase">{detailData.label}</span> 
                    </div>
                    <div className="text-right"><div className="text-[11px] font-black mb-1 opacity-40">출근 시간</div><div className="text-5xl font-black tracking-tighter">{detailData.reportTime || "--:--"}</div></div>
                </div>
                <div className="flex flex-col gap-4 mb-8">
                  {[{ t: '전반', con: detailData.content1 }, { t: '후반', con: detailData.content2 }].map((v, i) => v.con && (
                      <div key={i} style={{ backgroundColor: theme.card, borderColor: theme.border }} className="border rounded-[24px] p-5 flex items-center gap-4">
                          <div className="w-10 h-10 bg-blue-500 text-white rounded-xl flex items-center justify-center font-black text-[11px] shrink-0">{v.t}</div>
                          <div className="font-bold text-[16px] leading-relaxed">{v.con}</div>
                      </div>
                  ))}
                </div>
                ...
                <button onClick={() => setShowMemoInput(true)} className="fixed bottom-12 right-10 w-16 h-16 bg-blue-500 rounded-full shadow-2xl flex items-center justify-center z-[210]"><FileEdit className="w-7 h-7 text-white" /></button>
            </div>
        </div>
      )}

      {/* 설정 모달 수정 (X버튼 가시성 및 텍스트 반전) */}
      {showSettings && (
        <div className="fixed inset-0 bg-black/80 z-[250] flex items-center justify-center p-6" onClick={() => window.history.back()}>
          <div style={{ backgroundColor: theme.bg, color: theme.text }} className="w-full max-w-sm rounded-[40px] p-8 shadow-2xl overflow-y-auto max-h-[90vh]" onClick={e=>e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => window.history.back()} style={{ backgroundColor: theme.card }} className="p-2 rounded-full border"><X className="w-6 h-6" /></button>
            </div>
            <div className="space-y-6">
                <div style={{ backgroundColor: theme.card }} className="p-6 rounded-3xl border">
                ...
                <div style={{ backgroundColor: theme.card }} className="flex items-center justify-between p-6 rounded-3xl border">
                  ...
                  <button onClick={() => setIsDarkMode(!isDarkMode)} className={cn("w-14 h-8 rounded-full p-1 transition-all", isDarkMode?"bg-blue-600":"bg-slate-300")}><div className={cn("w-6 h-6 bg-white rounded-full transition-transform shadow-md", isDarkMode?"translate-x-6":"translate-x-0")} /></button>
                </div>
            </div>
          </div>
        </div>
      ...
      {showShiftMenu && (
        <div className="fixed inset-0 bg-black/60 z-[250] flex items-center justify-center p-6" onClick={() => setShowShiftMenu(false)}>
          <div style={{ backgroundColor: theme.card }} className="w-full max-w-sm rounded-3xl p-6 shadow-2xl grid grid-cols-2 gap-3" onClick={e=>e.stopPropagation()}>
            {RED_ITEMS.map(t => <button key={t} onClick={() => handleOverride(t, 'red')} className="py-3 bg-red-100 text-red-600 rounded-xl font-bold text-xs">{t}</button>)}
            {['지정근무', '대기충당', '교번교체', '휴무충당'].map(t => <button key={t} onClick={() => openModal(setShowDiaPicker.bind(null, {type: t, color: t === '휴무충당' ? 'yellow' : 'blue'}))} className="py-3 bg-slate-200 text-slate-800 rounded-xl font-bold text-xs">{t}</button>)}
            <button onClick={() => { const dk = format(selectedDate, 'yyyy-MM-dd'); const up = { ...overrides }; delete up[dk]; setOverrides(up); setDetailData(getShift(selectedDate, userAnchor.date, userAnchor.index, true, up)); setShowShiftMenu(false); }} className="col-span-2 py-3 bg-slate-100 text-slate-500 rounded-xl font-bold text-xs italic">원래 근무 복구</button>
          </div>
        </div>
      )}
      ...
      {showAddPeer && (
        <div className="fixed inset-0 bg-black/70 z-[300] flex items-center justify-center p-6" onClick={() => window.history.back()}>
          <div style={{ backgroundColor: theme.bg, color: theme.text }} className="w-full max-w-sm rounded-[40px] p-8 shadow-2xl" onClick={e=>e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8"><h2 className="text-2xl font-black">동료 등록</h2><button onClick={() => window.history.back()} style={{ backgroundColor: theme.card }} className="p-2 rounded-full border"><X className="w-6 h-6" /></button></div>
            <div className="space-y-6">
              ...
              <button onClick={() => { const n = (document.getElementById('pName') as HTMLInputElement).value; const d = (document.getElementById('pDate') as HTMLInputElement).value; const dia = (document.getElementById('pDia') as HTMLSelectElement).value; const findIdx = ROTATION_ORDER.indexOf(dia); if(n && findIdx !== -1) { setPeers([...peers, { id: Date.now(), name: n, anchorD: d, anchorI: findIdx }]); window.history.back(); } }} className="w-full bg-blue-500 text-white py-4 rounded-2xl font-black">등록 완료</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showEditPeers && (
        <div className="fixed inset-0 bg-black/70 z-[300] flex items-center justify-center p-6" onClick={() => window.history.back()}>
          <div style={{ backgroundColor: theme.bg, color: theme.text }} className="w-full max-w-sm rounded-[40px] p-8 shadow-2xl flex flex-col max-h-[75vh]" onClick={e=>e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black">동료 편집</h2><button onClick={() => window.history.back()} style={{ backgroundColor: theme.card }} className="p-2 rounded-full border"><X className="w-6 h-6" /></button></div>
            ...
            <button onClick={() => window.history.back()} className="w-full bg-slate-800 text-white py-4 rounded-2xl font-black mt-6">편집 완료</button>
          </div>
        </div>
      )}
    </div>

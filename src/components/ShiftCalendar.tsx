...
export const ShiftCalendar = () => {
  ...
  const theme = {
    ...
    highlight7: isDarkMode ? 'rgba(16, 185, 129, 0.12)' : 'rgba(16, 185, 129, 0.07)', // 7% 하이라이트
    btnPoint: '#475569',
  };
  ...
  const handleOverride = (type: string, color: string, dia?: string) => {
    ...
    setDetailData(getShift(selectedDate, userAnchor.date, userAnchor.index, true));
  ...
  return (
    <div style={{ backgroundColor: theme.bg, color: theme.text }} className="flex flex-col h-screen select-none font-sans overflow-hidden relative">
      ...
      <header className="flex items-center justify-between px-6 py-6 shrink-0 z-10">
        <div>
          <span style={{ color: theme.subText }} className="text-[11px] font-black mb-1 block uppercase tracking-widest">{format(currentMonth, 'yyyy년')}</span>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-black tracking-tighter">{activeTab === 'all' ? "전체근무" : format(currentMonth, 'M월')}</h1>
            {activeTab !== 'all' && (
              <div className="flex gap-1 ml-1">
                <button onClick={() => setCurrentMonth(addMonths(currentMonth, -1))} style={{ backgroundColor: theme.card }} className="p-2 border rounded-lg"><ChevronLeft className="w-4 h-4"/></button>
                <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} style={{ backgroundColor: theme.card }} className="p-2 border rounded-lg"><ChevronRight className="w-4 h-4"/></button>
              </div>
            )}
          </div>
        </div>
        <button onClick={() => openModal(setShowSettings)} style={{ backgroundColor: theme.card }} className="w-12 h-12 flex items-center justify-center border rounded-xl shadow-sm"><Settings style={{ color: theme.subText }} className="w-6 h-6" /></button>
      </header>

      {/* 메인 콘텐츠 */}
      <main className="flex-1 flex flex-col overflow-hidden">
        {activeTab === 'calendar' ? (
          <div className="flex-1 flex flex-col px-3 pb-4 h-full">
            <div className="grid grid-cols-7 mb-2 text-[11px] font-black opacity-60 text-center">
              ...
              {eachDayOfInterval({ start: startOfWeek(startOfMonth(currentMonth)), end: endOfWeek(endOfMonth(currentMonth)) }).map(d => {
                ...
                return (
                  <div key={d.toString()} onClick={() => { setSelectedDate(d); setDetailData(s); openModal(setShowDetail); }}
                       style={{ backgroundColor: theme.card, borderColor: theme.border, borderWidth: '0.5px', opacity: isCurr?1:0.35 }}
                       className="relative flex flex-col active:bg-slate-100 transition-colors overflow-hidden">
                    ...
                    {/* 다이아 번호: 13px, 상단 배치 */}
                    <div className="flex-1 flex flex-col items-start p-0.5 gap-0.5">
                      ...
                      <div className="flex flex-col gap-0.5 w-full">
                        {(memos[format(d, 'yyyy-MM-dd')] || []).slice(0, 3).map((m, i) => (<div key={i} style={{ color: theme.subText }} className="text-[10px] font-bold truncate px-0.5 leading-tight">{m}</div>))}
                      </div>
                    </div>
                  </div>
                );
          ...
          <div style={{ backgroundColor: theme.bg }} className="flex-1 flex flex-col h-full overflow-hidden relative">
            <div ref={peerScrollRef} className="flex-1 overflow-auto">
              <table style={{ borderColor: theme.border }} className="border-collapse w-max min-w-full text-center">
                <thead className="sticky top-0 z-20">
                  <tr>
                    <th style={{ backgroundColor: theme.card, color: theme.text, borderColor: theme.border }} className="p-4 border sticky left-0 z-30 text-[13px] font-black shadow-md">성명</th>
                    {eachDayOfInterval({ start: startOfMonth(currentMonth), end: endOfMonth(currentMonth) }).map(d => (
                      <th key={d.toString()} style={{ backgroundColor: isSameDay(d, new Date()) ? theme.highlight7 : theme.card, borderColor: theme.border, color: theme.text }} className="p-2 border text-[11px] min-w-[54px]">
                ...
                <tbody>
                  {[{ id: 0, name: '나', anchorD: userAnchor.date, anchorI: userAnchor.index, isUser: true }, ...peers].map(p => (
                    <tr key={p.id}>
                      <td style={{ backgroundColor: theme.card, borderColor: theme.border, color: theme.text }} className="p-4 border sticky left-0 z-10 text-[13px] font-black">{p.name}</td>
                      {eachDayOfInterval({ start: startOfMonth(currentMonth), end: endOfMonth(currentMonth) }).map(d => {
                        const s = getShift(d, p.anchorD, p.anchorI, p.isUser);
                        const standby = getStandbyLoc(s.content1);
                        return (
                          <td key={d.toString()} style={{ backgroundColor: isSameDay(d, new Date())?theme.highlight7: (s.diaNum === '~' ? '#D1D5DB' : (s.diaNum.includes('휴') ? (isDarkMode ? '#411C1C' : '#FEE2E2') : theme.card)), borderColor: theme.border }} className="p-2 border h-16 min-w-[54px] relative">
                            {standby && (<div style={{ color: getStandbyColor(standby) }} className="absolute top-0.5 left-1 font-black text-[11px] leading-none">{standby}</div>)}
            ...
            <div className="absolute bottom-6 right-6 flex flex-col gap-3 z-20">
              <button onClick={() => openModal(setShowAddPeer)} className="w-14 h-14 bg-blue-600 text-white rounded-full shadow-2xl flex items-center justify-center active:scale-95"><UserPlus className="w-6 h-6"/></button>
            </div>
          </div>
        ) : (
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex gap-1 p-2 bg-card border-b items-center overflow-x-auto no-scrollbar">
              ...
              {allSubTab === '교번' && (
                <div className="ml-auto flex items-center bg-slate-100 dark:bg-neutral-800 rounded-lg px-2 py-1 min-w-[80px]">
                  <Search className="w-3 h-3 opacity-40 mr-1"/>
                  <input placeholder="검색" className="bg-transparent text-[11px] outline-none w-full font-bold text-black dark:text-white" onChange={e=>setSearchQuery(e.target.value)} />
                </div>
              )}
            </div>
                  ...
                  {ROTATION_ORDER.map((dia, i) => {
                    ...
                    return (
                      <div key={i} style={{ backgroundColor: isHit ? theme.highlight7 : theme.card, borderColor: theme.border, borderWidth: '0.5px' }} className="aspect-square flex flex-col items-center justify-center relative">
                        <span className="text-[9px] opacity-30 absolute top-0.5 left-1">{i + 1}</span>
                        <span className={cn("text-[13px] font-black", dia.includes('휴') && "text-red-500")}>{dia}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <table className="w-full border-collapse">
                  <tbody style={{ color: theme.text }}>
                    {(allData?.[allSubTab==='평'?'wd':allSubTab==='휴'?'hd':allSubTab==='평평'?'ww':allSubTab==='평휴'?'wh':allSubTab==='휴휴'?'hh':'hw'] || []).map((row: any, i: number) => (
                      <tr key={i} style={{ backgroundColor: i % 2 === 0 ? theme.card : theme.bg }} className="text-center text-[11px] border-b border-black/5">
                        <td className="p-3 font-black">{row.matchNum}</td>
                        <td className="p-3 font-black">{row.reportTime}</td>
                        <td className="p-3 whitespace-pre-wrap">{row.content1}</td>
                        <td className="p-3 whitespace-pre-wrap">{row.content2}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {/* 하단바: 미니멀 테마 */}
      <nav style={{ backgroundColor: theme.card, borderColor: theme.border }} className="mx-6 mb-6 px-4 py-3 rounded-2xl flex items-center justify-between border shadow-lg relative z-10">
        {[
          { id: 'calendar', icon: CalendarIcon, label: "내 근무" },
          { id: 'peers', icon: Users, label: "동료 근무" },
          { id: 'all', icon: LayoutList, label: "전체 근무" }
        ].map((item) => {
          const isActive = activeTab === item.id;
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id as any)} 
                    style={{ color: isActive ? theme.text : theme.subText }}
                    className={cn("flex flex-col items-center transition-all duration-200", isActive && "scale-110")}>
      ...
      {showDetail && detailData && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center animate-in slide-in-from-bottom duration-300">
          <div style={{ backgroundColor: theme.bg, color: theme.text }} className="w-full h-full p-8 flex flex-col relative overflow-y-auto">
            <div className="flex justify-between items-start mb-10">
              ...
              <button onClick={() => window.history.back()} style={{ backgroundColor: theme.card }} className="p-3 rounded-full border shadow-sm"><X style={{ color: theme.text }} className="w-6 h-6"/></button>
            </div>
            <div style={{ backgroundColor: theme.card, borderColor: theme.border }} className="border rounded-[32px] p-10 flex justify-between items-center shadow-md mb-8">
              ...
              <div className="text-right">
                <div style={{ color: theme.subText }} className="text-[11px] font-black mb-1 opacity-60">출근 시간</div>
            ...
            <div className="flex flex-col gap-4 flex-1">
              {[{ t: '전반', con: detailData.content1 }, { t: '후반', con: detailData.content2 }].map((v, i) => v.con && (
                <div key={i} style={{ backgroundColor: theme.card, borderColor: theme.border }} className="border rounded-[24px] p-6 flex items-center gap-4">
              ...
              <div className="h-24 shrink-0" /> {/* 플로팅 버튼 여유 공간 */}
            </div>
            {detailData.isUser && (
              <button onClick={() => { window.history.pushState({ modal: true }, ""); setShowShiftMenu(true); }} style={{ backgroundColor: theme.card, color: theme.btnPoint, borderColor: theme.border }} className="mt-auto mb-6 px-5 py-4 rounded-2xl font-black text-[15px] border shadow-sm">근무 변경</button>
            )}
            <button onClick={() => setShowMemoInput(true)} style={{ backgroundColor: theme.btnPoint }} className="fixed bottom-10 right-10 w-16 h-16 rounded-full shadow-2xl flex items-center justify-center z-[250] active:scale-90"><FileEdit className="w-7 h-7 text-white" /></button>
          </div>
        </div>
      )}
      ...
      {/* ... 나머지 ShiftMenu, DiaPicker, AddPeer, EditPeers 로직 동일 보존 ... */}
      {showShiftMenu && (

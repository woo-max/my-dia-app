...
const getDutyColor = (duty: string, defaultColor: string) => {
  ...
  const d = duty.trim();
  if (d.includes('지정근무') || d.includes('대기충당') || d.includes('교번교체')) return '#2563EB';
  if (d.includes('휴무충당')) return '#D97706';
  return defaultColor;
};
...
export const ShiftCalendar = () => {
  ...
  useEffect(() => {
    const refreshData = async () => {
      try { const data = await fetchAllRotationData(); setAllData(data); } catch (e) { console.error("Data Fail"); }
      finally { setIsLoading(false); }
    };
    refreshData();
  }, []);

  // 동료 근무 오늘 날짜 정렬 (crash 방지 추가)
  useEffect(() => {
    if (activeTab === 'peers' && !isLoading && peerScrollRef.current) {
      setTimeout(() => {
        const todayIndex = differenceInDays(new Date(), startOfMonth(currentMonth));
        if (todayIndex >= 0 && peerScrollRef.current) {
          peerScrollRef.current.scrollLeft = todayIndex * 54;
        }
      }, 50);
    }
  ...
  return (
    <div style={{ backgroundColor: theme.bg, color: theme.text }} className="flex flex-col h-screen select-none font-sans overflow-hidden relative">
      ...
      >
        {activeTab === 'calendar' ? (
          <div className="flex-1 flex flex-col px-2 pb-2 h-full">
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden relative">
                  ...
                  {[{ id: 0, name: '나', anchorD: userAnchor.date, anchorI: userAnchor.index, isUser: true }, ...peers].map(p => (
                    <tr key={p.id}>
                      ...
                      {eachDayOfInterval({ start: startOfMonth(currentMonth), end: endOfMonth(currentMonth) }).map(d => {
                        ...
                        const standby = (allData && s.content1) ? ['동','불','기','오','진','사'].find(t => s.content1.includes(t)) : null;
                        return (
            ...
            <div className="absolute bottom-4 right-4"><button onClick={() => openModal(setShowAddPeer)} className="w-12 h-12 bg-blue-600 text-white rounded-full shadow-lg flex items-center justify-center active:scale-95"><UserPlus className="w-5 h-5"/></button></div>
          </div>
        ) : (
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="bg-card border-b flex flex-col gap-2 p-2">
              ...
              {allSubTab === '교번' && (
                <div className="flex items-center bg-slate-100 dark:bg-neutral-800 rounded-xl px-3 py-1.5 border border-black/5">
                  <Search className="w-3.5 h-3.5 opacity-40 mr-2"/><input placeholder="교번 검색..." className="bg-transparent text-xs outline-none w-full font-bold" onChange={e=>setSearchQuery(e.target.value)} />
                </div>
              )}
            </div>
              ...
              {allSubTab === '교번' ? (
                <div className="grid grid-cols-7 gap-0 border-t border-l" style={{ borderColor: theme.border }}>
                  {ROTATION_ORDER.map((dia, i) => (
                    <div key={i} style={{ backgroundColor: (searchQuery && dia.includes(searchQuery)) ? theme.highlight7 : theme.card, borderColor: theme.border, borderWidth: '0.5px' }} className="aspect-square flex flex-col items-center justify-center relative">
      ...
      <nav style={{ backgroundColor: theme.card, borderColor: theme.border }} className="mx-6 mb-6 px-4 py-2.5 rounded-2xl flex items-center justify-between border shadow-lg z-10 shrink-0">
        {[
          ...
          <button key={item.id} onClick={() => setActiveTab(item.id as any)} style={{ color: activeTab === item.id ? '#2563EB' : theme.subText }} className={cn("flex flex-col items-center transition-transform", activeTab === item.id && "scale-110")}>
      ...
      {showDetail && detailData && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center animate-in slide-in-from-bottom duration-300">
          <div style={{ backgroundColor: theme.bg, color: theme.text }} className="w-full h-full p-4 flex flex-col relative overflow-hidden">
            <div className="flex justify-end mb-1"><button onClick={() => window.history.back()} style={{ backgroundColor: theme.card }} className="p-2 rounded-full border"><X className="w-5 h-5"/></button></div>
            ...
            <div className="flex justify-between items-end mb-3 px-2">
              <div className="flex items-end gap-2">
                ...
                <span className="text-xs font-bold opacity-40 mb-1">{['일요일','월요일','화요일','수요일','목요일','금요일','토요일'][selectedDate.getDay()]}</span>
              </div>
              {detailData.isUser && ( 
                <button onClick={() => { window.history.pushState({ modal: true }, ""); setShowShiftMenu(true); }} className="text-[11px] font-black border border-blue-200 bg-blue-50 text-blue-600 px-2.5 py-1 rounded-lg active:scale-95">근무 변경</button> 
              )}
            </div>
            
            <div className="space-y-2 flex-1 overflow-y-auto pb-24 px-1">
              <div style={{ backgroundColor: theme.card, borderColor: theme.border }} className="border rounded-2xl p-3 flex justify-between items-center shadow-sm">
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-[9px] font-black opacity-40 uppercase tracking-widest">{detailData.label}</span>
                </div>
                <div className="text-right">
                  <div className="text-[10px] font-black opacity-30 mb-0.5 uppercase">출근 시간</div>
              ...
              {[{ t: '전반', con: detailData.content1 }, { t: '후반', con: detailData.content2 }].map((v, i) => v.con && (
                <div key={i} style={{ backgroundColor: theme.card, borderColor: theme.border }} className="border rounded-xl p-2.5 flex items-center gap-3">
                  <div className="w-7 h-7 bg-slate-100 dark:bg-neutral-800 rounded-lg flex items-center justify-center font-black text-[9px] text-slate-500 shrink-0">{v.t}</div>
                  <div className="font-bold text-[14px] leading-snug whitespace-pre-wrap">{v.con}</div>
                </div>
              ))}
            ...
            {/* 메모 추가 플로팅 버튼 */}
            <button onClick={() => setShowMemoInput(true)} className="fixed bottom-10 right-10 w-14 h-14 bg-slate-800 text-white rounded-full shadow-2xl flex items-center justify-center z-[250] active:scale-95"><FileEdit className="w-6 h-6" /></button>
          </div>
        </div>
      )}
      ...
      {showMemoInput && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
          ...
          <div style={{ backgroundColor: theme.card }} className="relative w-full max-w-sm rounded-3xl p-5 shadow-2xl flex items-center gap-3">
            ...
            <button onClick={() => { if(!memoInput.trim()) return; const dk = format(selectedDate, 'yyyy-MM-dd'); setMemos({ ...memos, [dk]: [...(memos[dk] || []), memoInput] }); setMemoInput(""); setShowMemoInput(false); }} className="bg-blue-600 text-white p-3.5 rounded-xl"><Check className="w-5 h-5"/></button>
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/80 z-[250] flex items-center justify-center p-6" onClick={() => window.history.back()}>
          <div style={{ backgroundColor: theme.bg, color: theme.text }} className="w-full max-w-sm rounded-[32px] p-6 shadow-2xl overflow-y-auto max-h-[80vh]">
            ...
            <div className="space-y-4 text-center">
                <div style={{ backgroundColor: theme.card }} className="p-6 rounded-2xl border">
                    <label className="block text-[10px] font-black uppercase mb-4 opacity-40">내 근무 기준</label>
                    ...
                    <select id="uDia" defaultValue={ROTATION_ORDER[userAnchor.index]} style={{ backgroundColor: theme.bg }} className="w-full p-3 rounded-xl font-bold text-center mb-4 border text-sm">{DIA_NUMBERS.map(n => <option key={n} value={n}>{n}</option>)}</select>
                    <button onClick={() => { const d = (document.getElementById('uDate') as HTMLInputElement).value; const dia = (document.getElementById('uDia') as HTMLSelectElement).value; const findIdx = ROTATION_ORDER.indexOf(dia); if(findIdx !== -1) { setUserConfig({ date: d, index: findIdx }); localStorage.setItem('shift_user_config', JSON.stringify({ date: d, index: findIdx })); window.history.back(); } }} className="w-full bg-slate-800 text-white py-3.5 rounded-xl font-black text-xs">기준 저장</button>
                </div>
      ...
      {showDiaPicker && (
        <div className="fixed inset-0 bg-black/95 z-[300] flex flex-col items-center justify-center p-8 overflow-y-auto">
          ...
          <div className="w-full bg-white rounded-3xl p-2 max-h-[60vh] overflow-y-auto grid grid-cols-3 gap-2">
            {[...Array.from({ length: 54 }, (_, i) => (i + 1).toString()), "~", "대1", "대2", "대3", "대4", "대5", "대6", "대11", "대12", "대13", "대14"].map(n => (<button key={n} onClick={() => handleOverride(showDiaPicker.type, showDiaPicker.color, n)} className="py-4 border rounded-xl font-black text-lg text-black active:bg-slate-200">{n}</button>))}
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 z-[300] flex items-center justify-center p-6" onClick={() => window.history.back()}>
          <div style={{ backgroundColor: theme.bg, color: theme.text }} className="w-full max-w-sm rounded-[32px] p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              <input id="pName" type="text" style={{ backgroundColor: theme.card }} className="w-full p-4 rounded-xl font-bold border" placeholder="이름" />
              <input id="pDate" type="date" defaultValue={format(new Date(), 'yyyy-MM-dd')} style={{ backgroundColor: theme.card }} className="w-full p-4 rounded-xl font-bold border" />
              <select id="pDia" style={{ backgroundColor: theme.card }} className="w-full p-4 rounded-xl font-bold border">{DIA_NUMBERS.map(n => <option key={n} value={n}>{n}</option>)}</select>
              <button onClick={() => { const n = (document.getElementById('pName') as HTMLInputElement).value; const d = (document.getElementById('pDate') as HTMLInputElement).value; const dia = (document.getElementById('pDia') as HTMLSelectElement).value; const findIdx = ROTATION_ORDER.indexOf(dia); if(n && findIdx !== -1) { setPeers([...peers, { id: Date.now(), name: n, anchorD: d, anchorI: findIdx }]); window.history.back(); } }} className="w-full bg-blue-600 text-white py-4 rounded-xl font-black shadow-lg">등록 완료</button>
            </div>
          </div>
        </div>
      )}

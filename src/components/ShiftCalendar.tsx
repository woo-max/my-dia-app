...
export const ShiftCalendar = () => {
  ...
  useEffect(() => {
    const handlePopState = () => {
      if (showDetail || showSettings || showAddPeer || showEditPeers || showShiftMenu) {
  ...
  }, [showDetail, showSettings, showAddPeer, showEditPeers, showShiftMenu, showMemoInput]);
  ...
  const theme = {
    ...
    todayHeaderCalendar: isDarkMode ? 'rgba(16, 185, 129, 0.3)' : 'rgba(16, 185, 129, 0.25)',
    ...
    offBg: isDarkMode ? '#411C1C' : '#FECACA', 
    emptyBg: isDarkMode ? '#2D2D2D' : '#D1D5DB', 
  };
  ...
  const getStandbyColor = (char: string) => {
    switch (char) { case '동': return '#22D3EE'; case '불': return '#22C55E'; case '기': return '#FF69B4'; default: return isDarkMode ? '#F3F4F6' : '#111827'; }
  };
  ...
  const SwipeableMemo = ({ text, onRemove }: { text: string, onRemove: () => void }) => {
    ...
    return (
      <div className="relative overflow-hidden rounded-2xl bg-red-500 mb-2 h-16 shrink-0">
        ...
        <div onTouchStart={e => setStartX(e.touches[0].clientX)} onTouchMove={e => setOffsetX(Math.min(0, e.touches[0].clientX - startX))} onTouchEnd={() => setOffsetX(offsetX < -80 ? -80 : 0)} style={{ transform: `translateX(${offsetX}px)` }} className="absolute inset-0 bg-slate-50 dark:bg-neutral-900 border border-black/5 flex items-center px-5 transition-transform duration-200">
  ...
  const peerStartDate = isCurrentMonth ? new Date() : startOfMonth(currentMonth);
      ...
      >
        {isLoading ? ( <div className="flex-1 p-4 grid grid-cols-7 gap-1.5 animate-pulse"> {Array.from({ length: 35 }).map((_, i) => ( <div key={i} className="aspect-[3/4] bg-slate-200 dark:bg-neutral-800 rounded-xl" /> ))} </div>
            ...
            <div className="flex-1 flex flex-col px-3 h-full overflow-hidden">
                ...
                <div className="flex-1 grid grid-cols-7 auto-rows-fr gap-1.5 pb-4">
                {eachDayOfInterval({ start: startOfWeek(startOfMonth(currentMonth)), end: endOfWeek(endOfMonth(currentMonth)) }).map(d => {
                    ...
                    return (
                        <div key={d.toString()} onClick={() => { setSelectedDate(d); setDetailData(s); openModal(setShowDetail); }}
                             style={{ backgroundColor: theme.card, borderColor: theme.border, borderWidth: '1px', opacity: isCurr ? 1 : 0.1 }}
                             className={cn("relative flex flex-col rounded-xl transition-all shadow-sm overflow-hidden active:scale-95 cursor-default font-sans")}>
                            <div style={{ backgroundColor: isT ? theme.todayHeaderCalendar : theme.subCard }} className="py-1 px-1.5 flex items-center justify-between border-b border-black/5 dark:border-white/5">
                                <span style={{ color: h.isH ? '#EF4444' : h.isSat ? SAT_BLUE : theme.text }} className="text-[13px] font-bold">{format(d, 'd')}</span>
                            </div>
                            <div className="flex-1 flex flex-col items-center justify-start pt-1 px-0.5 gap-0.5 overflow-hidden">
                                <span style={{ color: s.isRed ? '#EF4444' : theme.text }} className="text-[16px] font-black tracking-tighter leading-none">{s.diaNum}</span>
                                {(memos[dk] || []).length > 0 && <div className="w-6 h-0.5 bg-gray-300 dark:bg-gray-600 mt-1 mb-0.5" />}
                                <div className="flex-1 w-full flex flex-col gap-0.5 overflow-hidden">
                                  {(memos[dk] || []).map((m, idx) => (<div key={idx} className="w-full text-[10.5px] font-black text-black dark:text-white truncate text-center leading-tight">{m}</div>))}
                                </div>
                            </div>
                        </div>
                    );
            ...
            <div className="flex-1 flex flex-col h-full overflow-hidden relative font-sans">
                ...
                <table className="border-collapse w-max min-w-full font-sans text-center text-black dark:text-white">
                    <thead className="sticky top-0 z-20">
                        <tr>
                            <th style={{ backgroundColor: theme.card, borderColor: theme.border }} className="p-4 border sticky left-0 z-30 text-[13px] font-black shadow-md">성명</th>
                            {eachDayOfInterval({ start: peerStartDate, end: endOfMonth(currentMonth) }).map(d => {
                                ...
                                return ( <th key={d.toString()} style={{ color: h.isH ? '#EF4444' : h.isSat ? SAT_BLUE : (isDarkMode ? '#9CA3AF' : '#111'), borderColor: theme.border, backgroundColor: isT ? theme.todayHeaderPeers : theme.bg }} className="p-2 border text-[11px] min-w-[60px] relative font-sans text-center"> {isT && <span className="absolute -top-1 left-1/2 -translate-x-1/2 text-[7px] font-black bg-emerald-500 text-white px-1 rounded-sm shadow-sm">TODAY</span>} {format(d, 'd')}<br/>{['일','월','화','수','목','금','토'][d.getDay()]} </th> );
                            })}
                        </tr>
                    </thead>
                    <tbody>
                    {[{ id: 0, name: '나', anchorD: userAnchor.date, anchorI: userAnchor.index, isUser: true }, ...peers].map(p => (
                        <tr key={p.id}>
                        <td style={{ backgroundColor: theme.card, borderColor: theme.border }} className="p-4 border sticky left-0 z-10 text-[13px] font-black shadow-sm">{p.name}</td>
                        {eachDayOfInterval({ start: peerStartDate, end: endOfMonth(currentMonth) }).map(d => {
                            ...
                            <td key={d.toString()} onClick={() => { setSelectedDate(d); setDetailData(s); openModal(setShowDetail); }}
                                ...
                                <div style={{ color: isOff ? '#EF4444' : (isE ? (isDarkMode ? '#F3F4F6' : '#1F2937') : theme.text) }} className={cn("text-[15px] font-black", isT && "scale-110")}>{s.diaNum}</div>
                                {closingTime && (<div style={{ color: isDarkMode ? '#9CA3AF' : '#4B5563' }} className="absolute bottom-1 right-0.5 font-black text-[13px] italic leading-none">{closingTime}</div>)}
                            </td>
                            );
                        })}
                        </tr>
                ...
                <div className="flex-1 overflow-auto">
                    ...
                    <tbody className="divide-y dark:divide-white/5">
                        ...
                        <tr key={i} style={{ backgroundColor: theme.card, color: theme.text }} className="active:bg-blue-500/5 text-center text-[11px] font-medium opacity-90">
                            <td className="p-3 text-[13px] font-black text-black dark:text-white">{row.matchNum}</td>
      ...
      {showDetail && detailData && (
        <div className={cn("fixed inset-0 z-[200] flex transition-all duration-300 font-sans", activeTab === 'peers' ? "items-end justify-center" : "items-center justify-center")}>
            ...
            >
                ...
                {/* 2. 전체 내용 하강 (mt-12) */}
                <div className="flex-1 flex flex-col mt-12 font-sans relative">
                    {/* 3. 날짜/요일 및 근무변경 버튼 라인 */}
                    <div className="flex justify-between items-end mb-8 font-sans">
                      <div>
                        <span className="text-blue-500 font-black text-[11px] uppercase tracking-widest block mb-1">{format(selectedDate, 'M월 d일')}</span>
                        <h2 className="text-4xl font-black tracking-tighter text-black dark:text-white leading-none">
                          {['일요일','월요일','화요일','수요일','목요일','금요일','토요일'][selectedDate.getDay()]}
                        </h2>
                      </div>
                      
                      {/* ★ 근무변경 버튼: 세련된 Glass 톤으로 변경 ★ */}
                      {detailData.isUser && (
                        <button 
                          ...
                          className="px-5 py-3 rounded-2xl font-black text-[13px] border border-black/5 dark:border-white/5 shadow-sm active:scale-95 transition-all cursor-pointer"
                        >
                          근무 변경
                        </button>
                      )}
                    ...
                    {/* 4. 근무 카드 영역 */}
                    <div style={{ backgroundColor: theme.card, borderColor: theme.border }} className="border rounded-[32px] p-8 flex justify-between items-center shadow-sm text-black dark:text-white mb-6">
                        <div className="flex flex-col items-center gap-2"> 
                          <div style={{ backgroundColor: theme.subCard }} className="w-20 h-20 rounded-[24px] flex items-center justify-center border shadow-inner"> 
                            <span style={{ color: detailData.isRed ? '#EF4444' : theme.text, fontSize: detailData.diaNum?.length > 3 ? '18px' : detailData.diaNum?.length > 2 ? '22px' : '36px' }} className="font-black whitespace-nowrap">{detailData.diaNum}</span> 
                          </div> 
                          <span className="text-[11px] font-black uppercase opacity-50 tracking-wider"> {detailData.label} </span> 
                        </div>
                    ...
                    <div className="flex flex-col gap-4">
                      {[{ t: '전반', con: detailData.content1 }, { t: '후반', con: detailData.content2 }].map((v, i) => v.con && (
                          <div key={i} style={{ backgroundColor: theme.card, borderColor: theme.border }} className="border rounded-[24px] p-5 flex items-center gap-4 shadow-sm shrink-0 text-black dark:text-white">
                              ...
                              <div className={cn("font-bold leading-relaxed whitespace-pre-wrap break-words", activeTab === 'peers' ? "text-[16px]" : "text-[15px]")}>{v.con}</div>
                          </div>
                      ))}
                    </div>
                    ...
                    {/* 7. 플로팅 메모 추가 버튼 */}
                    <button onClick={() => setShowMemoInput(true)} style={{ backgroundColor: isDarkMode ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.8)' }} className="fixed bottom-10 right-10 w-16 h-16 rounded-full shadow-2xl flex items-center justify-center active:scale-90 transition-all z-[210] backdrop-blur-xl border border-black/5">
                      <FileEdit className="w-7 h-7 text-slate-500 dark:text-slate-400" />
                    </button>
                </div>
            </div>
        </div>
      ...
      {showMemoInput && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 animate-in zoom-in-95">
          ...
          <div style={{ backgroundColor: theme.card }} className="relative w-full max-w-sm rounded-[32px] p-6 shadow-2xl flex items-center gap-3" onClick={e=>e.stopPropagation()}>
            ...
            <button onClick={() => { if(!memoInput.trim()) return; const dk = format(selectedDate, 'yyyy-MM-dd'); setMemos({ ...memos, [dk]: [...(memos[dk] || []), memoInput] }); setMemoInput(""); setShowMemoInput(false); }} className="bg-slate-200 dark:bg-neutral-700 text-black dark:text-white p-4 rounded-2xl active:scale-95 shadow-lg"> <Check className="w-6 h-6" /> </button>
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[250] flex items-center justify-center p-6 animate-in fade-in" onClick={() => window.history.back()}>
          <div style={{ backgroundColor: theme.card }} className="w-full max-w-sm rounded-3xl p-8 shadow-2xl overflow-y-auto max-h-[90vh]" onClick={e=>e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8 font-sans"><h2 className="text-2xl font-black text-black dark:text-white">설정</h2><X onClick={() => window.history.back()} className="w-6 h-6 opacity-30 cursor-pointer" /></div>
            <div className="space-y-6 text-center font-sans">
                <div style={{ backgroundColor: theme.subCard }} className="p-6 rounded-2xl border border-black/5 dark:border-white/5 shadow-inner">
                    ...
                    <input id="uDate" type="date" defaultValue={userAnchor.date} className="w-full p-3 rounded-xl text-sm font-bold outline-none text-center mb-4 bg-slate-50 dark:bg-neutral-800 text-black dark:text-white border border-black/5" />
                    <select id="uDia" defaultValue={ROTATION_ORDER[userAnchor.index]} className="w-full p-3 rounded-xl text-sm font-bold outline-none text-center mb-4 bg-slate-50 dark:bg-neutral-800 text-black dark:text-white border border-black/5">{DIA_NUMBERS.map(n => <option key={n} value={n}>{n}</option>)}</select>
                ...
                <div style={{ backgroundColor: theme.subCard, borderColor: theme.border }} className="flex items-center justify-between p-5 rounded-2xl border">
                  ...
                  <button onClick={() => setIsDarkMode(!isDarkMode)} className={cn("w-12 h-7 rounded-full p-1 transition-all", isDarkMode?"bg-slate-600":"bg-slate-300")}><div className={cn("w-5 h-5 bg-white rounded-full transition-transform shadow-lg", isDarkMode?"translate-x-5":"translate-x-0")} /></button>
                </div>
            </div>
          </div>
        </div>

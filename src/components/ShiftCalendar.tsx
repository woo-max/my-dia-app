...
export const ShiftCalendar = () => {
  ...
  const getShift = useCallback((date: Date, anchorDStr: string, anchorI: number, isUser: boolean = false) => {
    ...
    // [심사위원 포인트 1] 야간 및 주간 데이터 판별 로직 정밀 복구
  ...
  // [심사위원 포인트 2] Black Screen 방지 및 즉각적인 상태 동기화 (2번 눌러야 하는 문제 해결)
  useEffect(() => {
    if (showDetail) {
      const freshData = getShift(selectedDate, userAnchor.date, userAnchor.index, true);
  ...
  const getStandbyLoc = (content1?: string) => {
    ...
    const targets = ['사', '진', '오', '기', '불', '동']; // 역순 추적 타겟
      ...
      >
        {activeTab === 'calendar' ? (
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden relative">
            <div ref={peerScrollRef} className="flex-1 overflow-x-auto no-scrollbar">
                      ...
                      {eachDayOfInterval({ start: startOfMonth(currentMonth), end: endOfMonth(currentMonth) }).map(d => {
                        ...
                        return (
                          <td key={d.toString()} onClick={() => { setSelectedDate(d); openPop(setShowDetail); }}
                              style={{ backgroundColor: isSameDay(d, new Date())?theme.highlight7: (s.diaNum === '~' ? '#D1D5DB' : (s.diaNum.includes('휴') ? (isDarkMode ? '#411C1C' : '#FEE2E2') : theme.card)), borderColor: theme.border }} className="p-2 border h-14 min-w-[54px] relative">
            ...
            <div className="bg-card border-b flex flex-col gap-2 p-2">
              ...
              {allSubTab === '교번' && (
                <div style={{ backgroundColor: theme.bg }} className="flex items-center rounded-xl px-3 py-1.5 border border-black/5 mx-1">
                  <Search className="w-3.5 h-3.5 opacity-40 mr-2"/><input placeholder="교번 검색 (예: 2)" className="bg-transparent text-xs outline-none w-full font-bold text-black dark:text-white" onChange={e=>setSearchQuery(e.target.value.trim())} />
                </div>
              )}
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-24">
            ...
            <div className="flex flex-col gap-1.5 mt-4">
              {(memos[format(selectedDate, 'yyyy-MM-dd')] || []).map((m, idx) => (
                <SwipeableMemo key={idx} text={m} onRemove={() => {
                  const dk = format(selectedDate, 'yyyy-MM-dd');
                  setMemos({ ...memos, [dk]: memos[dk].filter((_, i) => i !== idx) });
                }} />
              ))}
            </div>
          </div>
          ...
          <div style={{ backgroundColor: theme.bg, color: theme.text }} className="relative w-full max-w-sm rounded-[32px] p-6 shadow-2xl overflow-y-auto max-h-[80vh]" onClick={e=>e.stopPropagation()}>
            ...
            <div className="space-y-4">
                <div style={{ backgroundColor: theme.card }} className="p-5 rounded-2xl border border-black/5">
                    <label className="block text-[10px] font-black uppercase mb-3 text-center opacity-40">내 근무 기준 설정</label>
                    ...
                    <button onClick={() => {
                      const findIdx = ROTATION_ORDER.indexOf(uDia);
                      if (findIdx !== -1) {
                        setUserConfig({ date: uDate, index: findIdx });
                        localStorage.setItem('shift_user_config', JSON.stringify({ date: uDate, index: findIdx }));
                        window.history.back();
                      }
                    }} className="w-full bg-blue-600 text-white py-3 rounded-xl font-black text-xs">적용 및 저장</button>
                </div>
      ...
      {showShiftMenu && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6">
          ...
          <div style={{ backgroundColor: theme.card }} className="relative w-full max-w-sm rounded-2xl p-4 shadow-2xl grid grid-cols-2 gap-2" onClick={e=>e.stopPropagation()}>
            ...
            }} className="col-span-2 py-2.5 opacity-40 font-bold text-[10px] italic">기본 근무 복구</button>
          </div>
        </div>
      )}

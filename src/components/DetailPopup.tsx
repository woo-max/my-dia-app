...
export const DetailPopup = ({ selectedDate, detailData, theme, onClose, onShiftMenu, onMemoInput, memoList, onDeleteMemo }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-[500] flex flex-col animate-in slide-in-from-bottom duration-300" 
      ...
      <div className="flex justify-end p-4"><button onClick={onClose} className="p-2 rounded-full border bg-white/10 shadow-sm"><X className="w-6 h-6"/></button></div>
      <div className="flex-1 overflow-y-auto px-6 pb-24">
        <div className="flex justify-between items-end mb-4">
          ...
          {detailData.isUser && <button onClick={onShiftMenu} className="text-[11px] font-black border px-3 py-1.5 rounded-lg opacity-60">근무 변경</button>}
        </div>
        ...
        <div className="mt-4 flex flex-col gap-2">
          {memoList.map((m: string, i: number) => (
            <div key={i} className="bg-red-500 rounded-xl relative h-12 flex items-center overflow-hidden">
              <button onClick={() => onDeleteMemo(i)} className="absolute right-4"><Trash2 className="w-4 h-4 text-white"/></button>
      ...
      <button onClick={onMemoInput} className="fixed bottom-8 right-8 w-14 h-14 bg-slate-800 text-white rounded-full shadow-2xl flex items-center justify-center z-[510]"><FileEdit className="w-6 h-6"/></button>
    </div>
  );
};
